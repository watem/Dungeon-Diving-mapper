...
//	private transient HashMap<Coords, Node> nodesByCoord;
	...
	private double heightDistanceMultiplier = 1.;

	public Node addNode(int x, int y, int z) {
		Node n = new Node(x, y, z, this, (name + ": type:node: elem:" + elemsCreated++));
		getNodes().add(n);
		if (nodesById==null) {
			resetHashMaps();
		}
...
//		return nodesByCoord.get(c);
	...
	/**
	 * @param heightDistanceMultiplier the distanceMultiplier to set
	 */
	public void setHeightDistanceMultiplier(double heightDistanceMultiplier) {
		this.heightDistanceMultiplier = heightDistanceMultiplier;
	}

	public double getHeightDistanceMultiplier() {
		return heightDistanceMultiplier;
	}
